...
- SeriesID catalog sourced from LABSTAT flat files at `download.bls.gov/pub/time.series/{survey}/` — one `{survey}.series` file + code tables per survey (e.g., `cu.series` = 1.3MB, `ce.series` = 3.9MB, `ln.series` = 15MB; code tables such as `cu.area`, `jt.state`). Only the `{survey}.series` file and its code tables are needed, not the `{survey}.data.*` observation files (those are hundreds of MB). Harvested at startup into an on-disk SQLite index — ~160K series and ~44MB for the 12 default surveys; `bls_search_series` operates entirely offline against this index
...
- **Index version.** `createCatalogStore` stamps a catalog index version. A bump's migration clears the sync state's completion marker, so an index persisted by an earlier release re-harvests on its first boot after upgrade while it keeps serving; `writeState` cannot clear the marker (it COALESCEs `completed_at`). Bump the version whenever the harvest's output changes. Otherwise the index re-harvests on the first boot after it is older than `BLS_CATALOG_CACHE_TTL_HOURS`.
- **Search.** One FTS5 candidate query — the query tokens OR'd as prefixes, bm25-ordered, capped at 1,000 rows — unioned with an exact-SeriesID lookup and the headline `COMMON_SERIES`, then rescored in memory. The survey, seasonal, and area filters all run in that SQL, before the cap, so `totalCount` and `capped` describe the filtered pool. `area` is a case-insensitive substring of `area_name`, `title`, or `series_id`, bound as an escaped `LIKE` pattern (`%`, `_`, `\` match literally) through `store.raw()`, since `MirrorStore.query` has no substring operator; applied after the cap instead, a broad query's area rows fell past the 1,000th bm25 row and came back empty or partial. The same substring gate still runs in the rescore for the unioned headline rows; an exact SeriesID match scores before it and survives any area.
- **Ranking ties.** Without `area`, a national row (no decoded area, or `U.S. city average` / `Total US` / the opt-in OEWS `National`) outranks a state or metro row at equal score, because an omitted `area` asks for national series. Otherwise ties keep candidate order: exact id, headline, then bm25.
...
- **Indexed surveys.** The set of surveys a `survey` filter can match is read from the index (distinct `survey_abbr`) with the row total, once per load and after each harvest. It is not derived from `SURVEYS`: a survey whose download failed is absent, and an index inside its TTL keeps its last harvest's surveys even after `BLS_CATALOG_INCLUDE_OES` changes. A filter on any other code gets a notice listing the indexed codes.
...
| `BLS_CATALOG_CACHE_TTL_HOURS` | No | Catalog freshness window in hours — re-harvest once older. Default `168` (7 days). |
...
- **SeriesID catalog freshness.** The catalog index reflects the LABSTAT flat files at its last harvest, which runs at startup when the index is empty or older than `BLS_CATALOG_CACHE_TTL_HOURS` (default 7 days). New series or geographic area changes won't appear until the next harvest. `bls_search_series` may miss very recently added series.
