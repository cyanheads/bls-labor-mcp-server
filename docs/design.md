...
BLS missing-value rows preserve the raw `value: "-"` for round-tripping and expose `available: false`. Counts distinguish total period rows from available observations; formatted output says unavailable explicitly. DataCanvas rows add nullable `value_numeric`, so SQL can aggregate without casting the sentinel.
